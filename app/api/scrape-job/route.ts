...
function parseCompensation(jobPosting: Record<string, unknown>): {
  ...
  const unitText = (coerceText(valueObject?.unitText) || coerceText(baseSalary.unitText)).toLowerCase()

  const paymentAmount = minValue && maxValue ? `${minValue}-${maxValue}` : exactValue || minValue || maxValue
...
function inferWorkArrangement(jobPosting: Record<string, unknown>, html: string): string {
  ...
  const text = `${extractTitle(html)} ${extractMetaTagCandidates(html, ['description', 'og:description'])}`
    .toLowerCase()
    .trim()
...
function scrapeJobFromHtml(html: string, pageUrl: string): ScrapedJob {
  ...
  const description =
    coerceText(jobPosting?.description) ||
    inferDescription(html) ||
    ''
