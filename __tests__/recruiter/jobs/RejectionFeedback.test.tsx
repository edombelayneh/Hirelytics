...
// Shared mocks used across modal-level and page-level rejection flow tests.
...
vi.mock('firebase/firestore', () => ({
  ...
  // Return either application metadata or recruiter profile payload by collection path.
...
vi.mock('../../../app/components/job/ApplicantsTable', () => ({
  ApplicantsTable: ({
    ...
    <div data-testid='applicants-table'>
      ...
      {/* Mirrors the real user interaction: recruiter chooses Rejected for an applicant. */}
...
vi.mock('../../../app/components/ui/select', () => {
  ...
  // Flatten nested mocked SelectContent/SelectItem trees into native <option> elements.
...
describe('RejectionFeedbackModal', () => {
  ...
  it('fills the explanation from a suggestion and still allows recruiter edits before submit', async () => {
    ...
    // Recruiters can personalize the canned suggestion before sending.
  ...
  it('shows an inline error and preserves values when submit fails', async () => {
    ...
    const textarea = screen.getByLabelText(/additional explanation/i) as HTMLTextAreaElement
    fireEvent.change(textarea, {
      target: { value: 'Role has been filled by another candidate.' },
    })
    ...
    // Failed submit should not clear user input, so retry is low-friction.
    expect(textarea.value).toBe('Role has been filled by another candidate.')
    expect((screen.getByLabelText(/reason for rejection/i) as HTMLSelectElement).value).toBe(
      'Position filled'
    )
  })
})
...
describe('JobDetailsPage rejection feedback save', () => {
  ...
  it('saves rejection feedback to the applicant application document in Firestore', async () => {
    ...
    // Validate the exact Firestore write payload generated by the rejection flow.
    await waitFor(() => {
